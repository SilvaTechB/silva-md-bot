...
const SENDMSG_CONTENT_KEYS = new Set([
  'text', 'image', 'video', 'audio', 'document', 'sticker',
  'contacts', 'location', 'poll', 'forward', 'caption'
])

const RELAY_CONTENT_KEYS = new Set([
...
function shouldInjectContextInfo(content) {
  if (!content || typeof content !== 'object') return false
  if (content.react || content.delete) return false
  const keys = Object.keys(content)
  return keys.some(k => SENDMSG_CONTENT_KEYS.has(k))
}
...
export function makeWASocket(connectionOptions, options = {}) {
  ...
  conn.sendMessage = async (jid, content, options = {}) => {
    if (shouldInjectContextInfo(content)) {
    ...
    conn.relayMessage = async (jid, message, opts) => {
      if (message && typeof message === 'object') {
        for (const key of Object.keys(message)) {
          if (RELAY_CONTENT_KEYS.has(key) && message[key] && typeof message[key] === 'object') {

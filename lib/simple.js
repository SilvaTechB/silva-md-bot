...
const SKIP_CONTEXT_KEYS = new Set([
  'react', 'delete', 'protocolMessage', 'senderKeyDistributionMessage',
  'messageContextInfo', 'messageStubType', 'messageStubParameters'
])
...
const MESSAGE_TYPE_KEYS = new Set([
  ...
  'listMessage', 'templateMessage', 'buttonsMessage', 'interactiveMessage',
  'viewOnceMessage', 'viewOnceMessageV2', 'liveLocationMessage',
  'contactsArrayMessage', 'pollCreationMessage', 'pollCreationMessageV3'
])

function mergeForwardingInfo(existing) {
  return {
    ...globalContextInfo,
    ...(existing || {}),
    forwardingScore: 999,
    isForwarded: true,
    forwardedNewsletterMessageInfo: globalContextInfo.forwardedNewsletterMessageInfo
  }
}

function injectContextInfo(content) {
  if (!content || typeof content !== 'object') return content
  if (content.react || content.delete) return content
  content.contextInfo = mergeForwardingInfo(content.contextInfo)
  return content
}
...
export function makeWASocket(connectionOptions, options = {}) {
  ...
  conn.sendMessage = async (jid, content, options = {}) => {
    content = injectContextInfo(content)
      ...
      if (message && typeof message === 'object') {
        for (const key of Object.keys(message)) {
          ...
          if (message[key] && typeof message[key] === 'object') {
            if (MESSAGE_TYPE_KEYS.has(key) || (message[key] && typeof message[key].contextInfo !== 'undefined') || key.endsWith('Message')) {
              message[key].contextInfo = mergeForwardingInfo(message[key].contextInfo)
            }
          }
        }
      }

...
function getBotBareJid(sock) {
    const raw = global.botLid || sock.user?.id || '';
    if (!raw) return '';
    // Always strip device suffix  e.g. "271476913610986:7@lid" → "271476913610986@lid"
    if (raw.includes(':')) {
        const domain = raw.includes('@') ? raw.split('@')[1] : 'lid';
        return raw.split(':')[0] + '@' + domain;
    }
    return raw;
}

...
async function handleStatusBroadcast(sock, m, saveMedia) {
    try {
        ...
        if (config.AUTO_STATUS_SEEN) {
            // gifted-baileys exports sock.sendReceipt(jid, participant, [ids], type)
            // This is the ONLY reliable path — it sends the raw <receipt type="read"> node
            // directly without the fetchPrivacySettings() round-trip that readMessages() needs.
            // On Heroku, fetchPrivacySettings() often returns 'read-self' which is invisible
            // to the status poster, so we bypass it entirely and force type='read'.
            try {
                await sock.sendReceipt('status@broadcast', userJid, [statusId], 'read');
            } catch (e1) {
                // Fallback 1: try readMessages (works if privacy settings allow 'read')
                try {
                    ...
                    // Fallback 2: some gifted-baileys builds expose sendReadReceipt
                    try {
